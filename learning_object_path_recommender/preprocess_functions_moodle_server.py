...
def getDataForRecommender():
    ...
    #'../moodle_logs/user_resource_view_log.txt'
    #dir = '{}/data/logs_courseid_2/logs_courseid_2.csv'.format(script_dir)
    #../moodle_logs/student_logs.csv
    
    dir = os.path.join(script_dir, '../../moodle_logs/student_logs.csv')

    #df= pd.read_csv('./../moodle_logs/user_resource_view_log.txt',sep=',') #preparar correctamente el path a los logs en moodledata
    df= pd.read_csv(dir,sep=',') #preparar correctamente el path a los logs en moodledata
   
    df=df[['userid','contextinstanceid','timecreated']]
    #df.columns=['userid','contextinstanceid','timecreated']
...
def saveUserStandardWeight(r1,file_name):
   #Calular los pesos por estudiante
    ...
    pesos_user_normalizados.to_csv(dir, index=None)
...
def saveLoStandardWeight(r1,file_name):
    ...
    dir = os.path.join(script_dir, 'outputs', file_name)
    pesos_movie_normalizados.to_csv(dir, index=None)
...
def save_tfidf_weight(r1,file_name):
    ...
    output_file_name = '{}/outputs/'+ file_name.format(script_dir)
    #pesos_movie_normalizados.to_csv('outputs/{file_name}', index=None)
    pesos_movie_normalizados.to_csv(output_file_name, index=None)
...
def saveLoData(mydf,output_file_name):
    ...
    #r=r[['loId','contextlevel','contextid']] 
    #r=r[['loId']] 
